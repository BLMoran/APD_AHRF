...
# Rename Variables for INV_DAYONE
arf <- arf |> 
  mutate(INV_DAYONE = case_when(
      INV_DAYONE == "2"~ "No",
      INV_DAYONE == "1" ~ "Yes"))
...
# Recode Emergency Response Variable
arf <- arf |> 
  mutate(EMG_RSP_ADM = case_when(
    EMG_RSP_ADM == "2" ~ "No",
    EMG_RSP_ADM == "1" ~ "Yes"))
...
# Australia Vs New Zealand Demographics
table_anz <- arf |>  
  select(patientn = count_pt, AGE, age_cat, SEX, Apache2Score, Apache3Score, ANZRODRiskOfDeath, SOFA_Score, ap3diag_min, COVID_pneumonitis_derived, icu_admit_source, HospitalClassification, INV_IND, INV_DAYONE, NIV_IND, ECMO_IND, TRACHE_IND, AHRF, Country) |>  
    tbl_summary(by = Country, missing = "no",
            ...
            statistic = list(
              ...
              ap3diag_min ~ "{n} ({p}%)",
              icu_admit_source ~ "{n} ({p}%)")) |> 
  modify_header(stat_1 = "**Australia**", stat_2 = "**New Zealand**") |> 
  modify_spanning_header(all_stat_cols() ~ "**Country**") |> 
  add_overall(col_label = "**Overall**", last = FALSE) |> 
  bold_labels() |> 
  italicize_labels()
...
arf |> group_by(IcuAdmitYYYY,pf_cat) |>
  ...
  scale_y_continuous(breaks = seq(0, 100, by = 10))
```
## Pre-ICU Admission Duration over Time
```{r}
arf |> mutate(IcuAdmitYYYY = as.factor(IcuAdmitYYYY)) |> 
  ggplot(aes(x = IcuAdmitYYYY, y = as.numeric(PRE_ICU_HRS)))+
  geom_boxplot(outlier.shape = NA) +
  labs(x="Year", y="Pre-ICU Duration (Hours)", title="Pre-ICU Admission Hours over Time")+
  theme_bw() +
  theme(panel.grid.minor.x = element_blank(), panel.grid.minor.y = element_blank())+
  coord_cartesian(ylim = c(0, 100))
```
...
```
#### Unadjusted Proprtions of Patients Receiving Interventions Over Time- Graphical

::: panel-tabset
## Invasive Ventilation on Day 1 Overall
```{r, warning=FALSE, message=FALSE}
arf |> group_by(IcuAdmitYYYY, INV_IND) |>
  summarise(n = n()) |> 
  mutate(percent = 100*n/sum(n)) |> 
  filter(INV_IND=="Yes") |>
  ggplot()+
  geom_line(aes(x = IcuAdmitYYYY, y = percent))+
    labs(x="Year", y="Proportion (%)", title="Proportion of Patients Receving IPPV on Day 1 over Time", subtitle = "Overall")+
  theme_bw() +
  theme(panel.grid.minor.x = element_blank(), panel.grid.minor.y = element_blank())+
  scale_x_continuous(breaks = seq(2005, 2022, by = 2))
```

## Invasive Ventilation on Day 1 by AHRF Cat
```{r, warning=FALSE, message=FALSE}
arf |> group_by(IcuAdmitYYYY, pf_cat, INV_DAYONE) |>
  summarise(n = n()) |> 
  mutate(percent = 100*n/sum(n)) |> 
  filter(INV_DAYONE=="Yes") |>
  ggplot()+
  geom_line(aes(x = IcuAdmitYYYY, y = percent, color = pf_cat))+
    labs(x="Year", y="Proportion (%)", title="Proportion of Patients Receving IPPV on Day 1 over Time", subtitle = "by AHRF Category", color = "AHRF Category")+
  theme_bw() +
  theme(panel.grid.minor.x = element_blank(), panel.grid.minor.y = element_blank())+
  guides(color=guide_legend(override.aes=list(fill=NA)))+
...
## Invasive Ventilation After Day 1
```{r, warning=FALSE, message=FALSE}
arf |> group_by(IcuAdmitYYYY, pf_cat, INV_IND) |>
  summarise(n = n()) |> 
  mutate(percent = 100*n/sum(n)) |> 
  filter(INV_IND=="Yes") |>
  ggplot()+
  geom_line(aes(x = IcuAdmitYYYY, y = percent, color = pf_cat))+
    labs(x="Year", y="Proportion (%)", title="Proportion of Patients Receving IPPV on Any Day over Time", subtitle = "by AHRF Category", color = "AHRF Category")+
  theme_bw() +
  theme(panel.grid.minor.x = element_blank(), panel.grid.minor.y = element_blank())+
  guides(color=guide_legend(override.aes=list(fill=NA)))+
  scale_y_continuous(breaks = seq(0, 80, by = 5))+
...
```{r, warning=FALSE, message=FALSE}
arf |> group_by(IcuAdmitYYYY, pf_cat, NIV_IND) |>
  summarise(n = n()) |> 
  mutate(percent = 100*n/sum(n)) |> 
  filter(NIV_IND=="Yes") |>
  ggplot()+
  geom_line(aes(x = IcuAdmitYYYY, y = percent, color = pf_cat))+
    labs(x="Year", y="Proportion (%)", title="Proportion of Patients Receving NIV on Any Day over Time", subtitle = "by AHRF Category", color = "AHRF Category")+
  theme_bw() +
  theme(panel.grid.minor.x = element_blank(), panel.grid.minor.y = element_blank())+
  guides(color=guide_legend(override.aes=list(fill=NA)))+
  scale_y_continuous(breaks = seq(0, 80, by = 5))+
...
## ECMO
```{r, warning=FALSE, message=FALSE}
arf |> group_by(IcuAdmitYYYY, pf_cat, ECMO_IND) |>
  summarise(n = n()) |> 
  mutate(percent = 100*n/sum(n)) |> 
  filter(ECMO_IND=="Yes") |>
  ggplot()+
  geom_line(aes(x = IcuAdmitYYYY, y = percent, color = pf_cat))+
    labs(x="Year", y="Proportion (%)", title="Proportion of Patients Receving ECMO on Any Day over Time", subtitle = "by AHRF Category", color = "AHRF Category")+
  theme_bw() +
  theme(panel.grid.minor.x = element_blank(), panel.grid.minor.y = element_blank())+
  guides(color=guide_legend(override.aes=list(fill=NA)))+
  scale_y_continuous(breaks = seq(0, 80, by = 1))+
  scale_x_continuous(breaks = seq(2005, 2022, by = 2))
```
:::

#### Unadjusted Proprtions of Patients Receiving Interventions Over Time- Tabular
```{r, warning=FALSE, message=FALSE}
...
inv_ind <- arf_int |> 
  group_by(time_cat, INV_IND) |>
  summarise(n = n()) |> 
  mutate(percent = 100*n/sum(n)) |> 
  filter(INV_IND=="Yes") |> 
    mutate(across(percent, ~round(.,4))) |> 
    select(time_cat, percent)

inv_dayone <- arf_int |> 
    group_by(time_cat, INV_DAYONE) |>
  summarise(n = n()) |> 
  mutate(percent = 100*n/sum(n)) |> 
  filter(INV_DAYONE=="Yes") |> 
    mutate(across(percent, ~round(.,4))) |> 
...
arf_prop <- cbind(inv_ind, inv_dayone, niv_ind, ecmo_ind)
...
# Original Categories
time_death_km <-  
  survfit2(Surv(time, status) ~ strata(pf_cat), data=arf) |> 
  ggsurvfit(linewidth = 1) +
  add_confidence_interval() +
  coord_cartesian(xlim = c(0, 365)) +
  labs(title = "Time to Death", subtitle = "By AHRF Categories", x = "Time (Days)", y = "Survival Probability") +
  theme(legend.position = "right") +
  add_legend_title("AHRF Category")

time_death_timecat_km <-  
  survfit2(Surv(time, status) ~ strata(time_cat), data=arf) |> 
  ggsurvfit(linewidth = 1) +
  add_confidence_interval() +
  coord_cartesian(xlim = c(0, 365)) +
  labs(title = "Time to Death", subtitle = "By Time Categories", x = "Time (Days)", y = "Survival Probability") +
  theme(legend.position = "right") +
  add_legend_title("Time Category")

time_death_timecat_none_km <-  
  survfit2(Surv(time, status) ~ strata(time_cat), data=arf_none) |> 
  ggsurvfit(linewidth = 1) +
  add_confidence_interval() +
  coord_cartesian(xlim = c(0, 365)) +
  labs(title = "Time to Death in Patients with No AHRF", subtitle = "By Time Categories", x = "Time (Days)", y = "Survival Probability") +
  theme(legend.position = "right") +
  add_legend_title("Time Category")

time_death_timecat_mild_km <-  
  survfit2(Surv(time, status) ~ strata(time_cat), data=arf_mild) |> 
  ggsurvfit(linewidth = 1) +
  add_confidence_interval() +
  coord_cartesian(xlim = c(0, 365)) +
  labs(title = "Time to Death in Patients with Mild AHRF", subtitle = "By Time Categories", x = "Time (Days)", y = "Survival Probability") +
  theme(legend.position = "right") +
  add_legend_title("Time Category")

time_death_timecat_mod_km <-  
  survfit2(Surv(time, status) ~ strata(time_cat), data=arf_mod) |> 
  ggsurvfit(linewidth = 1) +
  add_confidence_interval() +
  coord_cartesian(xlim = c(0, 365)) +
  labs(title = "Time to Death in Patients with Moderate AHRF", subtitle = "By Time Categories", x = "Time (Days)", y = "Survival Probability") +
  theme(legend.position = "right") +
  add_legend_title("Time Category")

time_death_timecat_sev_km <-  
  survfit2(Surv(time, status) ~ strata(time_cat), data=arf_sev) |> 
  ggsurvfit(linewidth = 1) +
  add_confidence_interval() +
  coord_cartesian(xlim = c(0, 365)) +
  labs(title = "Time to Death in Patients with Severe AHRF", subtitle = "By Time Categories", x = "Time (Days)", y = "Survival Probability") +
  theme(legend.position = "right") +
  add_legend_title("Time Category")
...
## IPPV at Day 1
```{r}
arf |> filter(!(INV_DAYONE== "NULL")) |> 
ggplot(aes(x=pf_cont, y=pred_freqmodel, color = INV_DAYONE))+
  geom_smooth(method = "glm", formula = y ~ splines::ns(x, 4), se=TRUE)+
  labs(x=expression(paste(PaO[2]:FiO[2], " Ratio")), y="Conditional Probability of Hospital Mortality", title="Association of Day 1 PF Ratio and Hospital Mortality", 
       subtitle = "By Invasive Ventilation Status on Day 1 of ICU Admission", color = "Invasive Ventilation on Day 1")+
  theme_bw() +
  theme(panel.grid.major.x = element_blank())+
  guides(color=guide_legend(override.aes=list(fill=NA)))
```
## IPPV after Day 1
```{r}
arf |> mutate(INV_AFTER_D1 = case_when(
  INV_DAYONE == "No" & INV_IND == "Yes" ~ "Yes",
  INV_DAYONE == "No" & INV_IND == "No" ~ "No")) |> 
  drop_na(INV_AFTER_D1) |> 
ggplot(aes(x=pf_cont, y=pred_freqmodel, color = INV_AFTER_D1))+
  geom_smooth(method = "glm", formula = y ~ splines::ns(x, 4), se=TRUE)+
  labs(x=expression(paste(PaO[2]:FiO[2], " Ratio")), y="Conditional Probability of Hospital Mortality", title="Association of Day 1 PF Ratio and Hospital Mortality", 
       subtitle = "By Invasive Ventilation Status After Day 1 of ICU Admission", color = "Invasive Ventilation After Day 1")+
  theme_bw() +
  theme(panel.grid.major.x = element_blank())+
  guides(color=guide_legend(override.aes=list(fill=NA)))
```
## IPPV at Day 1 vs IPPV Thereafter
```{r}
arf |> mutate(INV_TIME = case_when(
  INV_DAYONE == "No" & INV_IND == "Yes" ~ "After Day 1",
  INV_DAYONE == "Yes" ~ "Day 1")) |> 
  drop_na(INV_TIME) |> 
ggplot(aes(x=pf_cont, y=pred_freqmodel, color = INV_TIME))+
  geom_smooth(method = "glm", formula = y ~ splines::ns(x, 4), se=TRUE)+
  labs(x=expression(paste(PaO[2]:FiO[2], " Ratio")), y="Conditional Probability of Hospital Mortality", title="Association of Day 1 PF Ratio and Hospital Mortality", 
       subtitle = "By Invasive Ventilation Status Timing", color = "Invasive Ventilation Timing")+
  theme_bw() +
  theme(panel.grid.major.x = element_blank())+
  guides(color=guide_legend(override.aes=list(fill=NA)))
```
...
## MET Call Admission
```{r}
arf |> drop_na(EMG_RSP_ADM) |> 
ggplot(aes(x=pf_cont, y=pred_freqmodel, color = EMG_RSP_ADM))+
  geom_smooth(method = "glm", formula = y ~ splines::ns(x, 4), se=TRUE)+
  labs(x=expression(paste(PaO[2]:FiO[2], " Ratio")), y="Conditional Probability of Hospital Mortality", title="Association of Day 1 PF Ratio and Hospital Mortality",
       subtitle = "By MET Call Admission", color = "MET Call Admission")+
  theme_bw() +
  theme(panel.grid.major.x = element_blank())+
  guides(color=guide_legend(override.aes=list(fill=NA)))
```
## COVID Pneumonitis
```{r}
arf |> filter(IcuAdmitYYYY>2020) |> 
  mutate(covid = case_when(
  COVID_pneumonitis_derived == 1 ~ "COVID Pneumonitis",
  COVID_pneumonitis_derived == 2 ~ "Probable COVID (or Exacerbated Condition)",
  COVID_pneumonitis_derived == 3 ~ "Incidental COVID",
  COVID_pneumonitis_derived == 0 ~ "No COVID")) |> 
ggplot(aes(x=pf_cont, y=pred_freqmodel, color = covid))+
  geom_smooth(method = "glm", formula = y ~ splines::ns(x, 4), se=TRUE)+
  labs(x=expression(paste(PaO[2]:FiO[2], " Ratio")), y="Conditional Probability of Hospital Mortality", title="Association of Day 1 PF Ratio and Hospital Mortality",
       subtitle = "By COVID Status (2020-2022)", color = "COVID Status")+
  theme_bw() +
  theme(panel.grid.major.x = element_blank())+
  guides(color=guide_legend(override.aes=list(fill=NA)))
```
...
# IPPV (on day one) Vs No-IPPV
arf_ippv_d1 <- arf |> filter(!(INV_DAYONE== "NA")) 

# Run MLM
arf_model_ippv_d1 <- glmer(DIED_HOSP ~ 0 + IcuAdmitYYYY + pf_cont + INV_DAYONE  + Apache3Score + CHR_RESP + CHR_CVS + (1|SiteID),
                             family = binomial(link = "logit"), data = arf_ippv_d1, nAGQ = 0,
                             glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)))

# G-computation
ippv_d1_avgcomp <- avg_comparisons(arf_model_ippv_d1, variables = "INV_DAYONE", comparison="difference", type="response")

# IPPV (after day one) Vs No-IPPV
arf_ippv_after_d1 <- arf |> mutate(INV_AFTER_D1 = case_when(
  INV_DAYONE == "No" & INV_IND == "Yes" ~ "Yes",
  INV_DAYONE == "No" & INV_IND == "No" ~ "No")) |> 
  drop_na(INV_AFTER_D1)

# Run MLM
arf_model_ippv_after_d1 <- glmer(DIED_HOSP ~ 0 + IcuAdmitYYYY + pf_cont + INV_AFTER_D1  + Apache3Score + CHR_RESP + CHR_CVS + (1|SiteID),
                             family = binomial(link = "logit"), data = arf_ippv_after_d1, nAGQ = 0,
                             glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)))

# G-computation
ippv_after_d1_avgcomp <- avg_comparisons(arf_model_ippv_after_d1, variables = "INV_AFTER_D1", comparison="difference", type="response")

# IPPV D1 vs IPPV After D1
arf_IPPV_d1vsafter <- arf |> 
...
# Run MLM
arf_model_ippv_d1vsafter <- glmer(DIED_HOSP ~ 0 + IcuAdmitYYYY + pf_cont + INV_TIME  + Apache3Score + CHR_RESP + CHR_CVS + (1|SiteID),
                             family = binomial(link = "logit"), data = arf_IPPV_d1vsafter, nAGQ = 0,
                             glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)))

# G-computation
ippv_d1vsafter_avgcomp <- avg_comparisons(arf_model_ippv_d1vsafter, variables = "INV_TIME", comparison="difference", type="response")
...
# MET Call Admission
# Run MLM
arf_model_met <- glmer(DIED_HOSP ~ 0 + IcuAdmitYYYY + pf_cont + EMG_RSP_ADM  + Apache3Score + CHR_RESP + CHR_CVS + (1|SiteID),
                             family = binomial(link = "logit"), data = arf, nAGQ = 0,
                             glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)))
# G-computation
met_avgcomp <- avg_comparisons(arf_model_met, variables = "EMG_RSP_ADM", comparison="difference", type="response")

## COVID Status
arf_covid <- arf |> filter(IcuAdmitYYYY>2020) |> 
  mutate(covid = case_when(
  COVID_pneumonitis_derived == 1 ~ "COVID Pneumonitis",
  COVID_pneumonitis_derived == 2 ~ "Probable COVID (or Exacerbated Condition)",
  COVID_pneumonitis_derived == 3 ~ "Incidental COVID",
  COVID_pneumonitis_derived == 0 ~ "No COVID")) |> 
  # Refactor to make "No COVID" the comaprator
  mutate(covid = factor(covid, levels = c("No COVID", "COVID Pneumonitis", "Probable COVID (or Exacerbated Condition)", "Incidental COVID")))

# Run MLM
arf_model_covid <- glmer(DIED_HOSP ~ 0 + IcuAdmitYYYY + pf_cont + covid  + Apache3Score + CHR_RESP + CHR_CVS + (1|SiteID),
                             family = binomial(link = "logit"), data = arf_covid, nAGQ = 0,
                             glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)))
# G-computation
covid_avgcomp <- avg_comparisons(arf_model_covid, variables = "covid", comparison="difference", type="response")
...
table <- function(data){
  df <- data |> select(Subgroup, contrast, estimate, conf.low, conf.high, p.value)|> 
    ...
    reframe(Subgroup, Contrast = contrast, "AME [95% CI]" = str_c(estimate, " [", conf.low, ", ", conf.high, "]"), "p-value" = p.value)
}

# Create dataframe
ippv_any <- ippv_avgcomp |> mutate(Subgroup = "IPPV (Any Time) Vs No-IPPV") |> table()
ippv_d1 <- ippv_d1_avgcomp |> mutate(Subgroup = "IPPV (Day 1) Vs No-IPPV") |> table()
ippv_after_d1 <- ippv_after_d1_avgcomp |> mutate(Subgroup = "IPPV (After Day 1) Vs No-IPPV") |> table()
ippv_d1vsafter <- ippv_d1vsafter_avgcomp |> mutate(Subgroup = "IPPV Day 1 Vs IPPV After Day 1") |> table()
...
metcall <- met_avgcomp |> mutate(Subgroup = "MET Call Admission") |> table()
covid <- covid_avgcomp |> mutate(Subgroup = "COVID Status") |> table()

# Bind all groups
subgroup_df <- rbind(ippv_any, ippv_d1, ippv_after_d1, ippv_d1vsafter, niv, ecmo, hosptype, sex, age, diag, rxlim, metcall, covid) 
...
Subgroup_table <-  subgroup_df |> 
  ...
  tab_footnote(footnote = "AME = Average Marginal Effect")
```
...
### Trends of Interventions Over Time
:::panel-tabset
## IPPV
```{r, warning=FALSE}
# Using IPPV at any time
arf |> filter(!(INV_IND== "NULL")) |> 
ggplot(aes(x=IcuAdmitYYYY, y=pred_hosp_mort, color = INV_IND))+
  geom_smooth(method = "glm", formula = y ~ splines::ns(x, 4), se=TRUE)+
  labs(x="Year", y="Conditional Probability of Hospital Mortality", title="Mortality Trend of Patients Receiving IPPV Over Time", 
       color = "Invasive Ventilation")+
  theme_bw() +
  theme(panel.grid.major.x = element_blank())+
  guides(color=guide_legend(override.aes=list(fill=NA)))+
  coord_cartesian(ylim=c(0,0.2))
...
```{r, warning=FALSE}
arf |> drop_na(NIV_IND) |> 
ggplot(aes(x=IcuAdmitYYYY, y=pred_hosp_mort, color = NIV_IND))+
  geom_smooth(method = "glm", formula = y ~ splines::ns(x, 4), se=TRUE)+
  labs(x="Year", y="Conditional Probability of Hospital Mortality", title="Mortality Trend of Patients Receiving NIV Over Time", 
       color = "Non-Invasive Ventilation")+
  theme_bw() +
  theme(panel.grid.major.x = element_blank())+
  guides(color=guide_legend(override.aes=list(fill=NA)))+
...
## ECMO
```{r,warning=FALSE}
arf |> drop_na(ECMO_IND) |> 
ggplot(aes(x=IcuAdmitYYYY, y=pred_hosp_mort, color = ECMO_IND))+
  geom_smooth(method = "glm", formula = y ~ splines::ns(x, 4), se=TRUE)+
  labs(x="Year", y="Conditional Probability of Hospital Mortality", title="Mortality Trend of Patients Receiving ECMO Over Time", 
       color = "ECMO")+
  theme_bw() +
  theme(panel.grid.major.x = element_blank())+
  guides(color=guide_legend(override.aes=list(fill=NA)))+
...
arf |> mutate(vent_sup = case_when(
  ...
  NIV_IND == "Yes" ~ "NIV",
  .default = "None")) |>  
  ggplot(aes(x=IcuAdmitYYYY, y=pred_hosp_mort))+
  geom_smooth(aes(color = "Overall"), method = "glm", formula = y ~ splines::ns(x, 4), se=TRUE, linetype = 4)+
  geom_smooth(aes(x=IcuAdmitYYYY, y=pred_hosp_mort, color = vent_sup), method = "glm", formula = y ~ splines::ns(x, 4), se=TRUE)+
  labs(x="Year", y="Conditional Probability of Hospital Mortality", title="Hospital Mortality over Time", color = "Ventilatory Support")+
  theme_bw() +
  theme(panel.grid.major.x = element_blank())+
  guides(color=guide_legend(override.aes=list(fill=NA, linetype = c(1,1,1,1,4))))+
  scale_color_discrete(limits = c("ECMO", "IPPV","NIV", "None", "Overall"))
