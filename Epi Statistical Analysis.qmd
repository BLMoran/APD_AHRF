...
arf <- arf |>  mutate(icu_admit_source = case_when(
            ...
            (ICU_SRCE ==5) ~ "Ward, Other Hospital",
            (ICU_SRCE ==6) ~ "ICU, Other Hopsital",
            (ICU_SRCE ==9) ~ "Direct from Home")) |> 
  select(-ICU_SRCE)

# Reorder Admission Source
arf <- arf |>  mutate(icu_admit_source = factor(icu_admit_source, levels = c("Emergency Department", "Operating Theatre/Recovery", "Ward", "ICU, Same Hospital", "Ward, Other Hospital", "ICU, Other Hospital", "Direct from Home")))
...
arf <- arf[complete.cases(arf$SMOKINGINTENSITY),]
...
# Generate Table 1
t1 <- arf |>  
  select(patientn = count_pt, AGE, age_cat, SEX, Apache2Score, Apache3Score, ANZRODRiskOfDeath, ap3diag_min, COVID_pneumonitis_derived, icu_admit_source, HospitalClassification, pf_cat) |>  
...
### Causes of AHRF
Below are the causes of AHRF, ordered by the most common.
...
#### All Outcomes
Below are the unadjusted outcomes for the different AHRF categories. The first table has the 4 categories (none, mild, moderate, severe) and the second table has the additional subgroup (moderate-severe).
```{r, include=FALSE, cache.lazy=FALSE}

# Generate Table of all outcomes
t_outcome <- arf |> 
...
arr <- function(plot){
  ...
  # Find values for varying values
  smooth_lci <- (smooths$y[1] - smooths$se[1]*qnorm(0.975))
  smooth_uci <- (smooths$y[1] + smooths$se[1]*qnorm(0.975))
  
  # Get 2005 and 2022 values (with SEs)
  mean_2005 <- smooths$y[1]*100
  conf.low_2005 <- smooth_lci*100
  conf.high_2005 <- smooth_uci*100
...
### Subgroup Analysis (Adjusted Analysis)
